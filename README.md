...
При запуске через Docker могут возникнуть ошибки записи в папки `data/raw` и `data/processed`.

Это связано с тем, что контейнер работает под Linux-пользователем, а папка может быть создана на хосте (Windows/WSL/Linux) с другими правами доступа.
